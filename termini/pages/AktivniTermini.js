...
import { StyleSheet, Text, View ,Image, ActionSheetIOS, TouchableOpacity,ScrollView,Platform} from 'react-native';
...
export default function Complete({navigation}) { 
    ...
    const ruta = useSelector(state=>state.rutaSudjiOrg)
    ...
    async function fetchaj(email) {
        fetch(URL+"/terminiUKojimaSudjelujem/"+email).then(res=>res.json()).then(data=>{setEventiSudj(data)})
        fetch(URL+"/terminiKojeOrganiziram/"+email).then(res=>res.json()).then(data=>{setEventiOrg(data);console.log(data)})
    ...
    <View style={styles.header}>
        ...
        padding:10} : styles.neodabran}>
                    <Text style={{fontWeight:"bold"}}>TERMINI U KOJIMA SUDJELUJEM</Text>
                </View> 
            </TouchableOpacity>  
        ...
        borderRadius:5,
        padding:10} : styles.neodabran}>
                    <Text style={{fontWeight:"bold"}}>TERMINI KOJE ORGANIZIRAM</Text>
                </View>
            </TouchableOpacity>
        </View>
...
{eventiOrg.map((dog)=>{return <DetaljiBox key={dog.eventid} navigation={navigation} dog={dog} vrsta="org"/> })}
                ...
                {sudjelujem && <View style={styles.actualFeed}>
                {eventiSudj.map((dog)=>{return <DetaljiBox key={dog.eventid} navigation={navigation} dog={dog} vrsta="sudj"/> })}
