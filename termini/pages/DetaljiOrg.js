...
import { StyleSheet, Text, View ,Image, ActionSheetIOS,Button, TouchableOpacity,ScrollView,Modal} from 'react-native';
...
import { darkmode,lightmode,NEW, NEWSudjiOrd } from '../redux/actions';
...
export default function Complete({navigation}) {
    ...
    const ruta=useSelector(state=>state.rutaEdit)
    ...
    async function deleteEvent() {
        fetch(URL+"/deleteEvent/"+eventInfo.eventid)

    }

    async function fetchaj(id) {
        fetch(URL+"/detalji/"+id+"/"+navigation.state.params.email).then(res=>res.json()).then(data=>{setSudionici(data.sudionici);setEventInfo(data.eventInfo);checkPridruzen(data.sudionici)})
    ...
    justifyContent:"center"}}>
        ...
        <TouchableOpacity onPress={()=>{navigation.navigate("AktivniTermini");dispatch(NEWSudjiOrd())}}>
         <Image style={styles.tinyLogoArrow} source={require("../assets/back.png")}/> 
         </TouchableOpacity>
        ...
        <View style={styles.naslov}>
            <View style={styles.naslov2}>
            <Text style={{fontSize:20 ,color:theme=="light"?"black":"white",fontWeight:theme=="light"?"":"bold"}}>{eventInfo.sport}</Text>
            <TouchableOpacity onPress={toggleModal}>
            <Image style={styles.smece} source={require("../assets/trash.png")} />
            </TouchableOpacity>            
        </View>
        </View>
            <View style={{flexDirection:"row",justifyContent:"center",width:"80%",backgroundColor:theme=="light"?"white":"#28559a",borderRadius:5,padding:3}}>
                ...
                <View style={styles.pola}>
                    <TouchableOpacity onPress={()=>{navigation.navigate("EditInfo",{eventInfo})}}style={{width:100,height:20,backgroundColor:"black",borderRadius:20,position:"absolute",right:"0%",top:"100%",justifyContent:"center",alignItems:"center"}}><Text style={{color:"white"}}>EDIT</Text></TouchableOpacity>
                <View style={styles.slikatext}>
            ...
            <Text style={{color:theme=="light" ? "black":"white",marginTop:"2%",fontSize:18,fontWeight:theme=="light"?"":"bold"}}>SUDIONICI</Text>
            <ScrollView style={{height:"20%",backgroundColor:theme=="light"?"white":"#28559a",width:"80%",marginTop:"2%",borderColor:"gray",borderWidth:1,borderRadius:5}}>
            ...
            <TouchableOpacity onPress={()=>{navigation.navigate("EditOpis",{"opis":eventInfo.opis,"eventid":eventInfo.eventid})}} style={{width:100,height:20,backgroundColor:"black",marginTop:20,borderRadius:20,justifyContent:"center",alignItems:"center"}}><Text style={{color:"white"}}>EDIT</Text></TouchableOpacity>
            
            <ScrollView style={{height:"20%",backgroundColor:theme=="light"?"white":"#28559a",width:"80%",marginTop:0,borderColor:"gray",borderWidth:1,borderRadius:5}}>
            
           
            <View style={styles.opis}> 
                
                <Text style={{color:theme=="light" ? "black":"white"}}>{eventInfo.opis}</Text>
                </View>
            </ScrollView>
...
>
  <View style={styles.centeredView}>
    <View style={styles.modalView}>
      ...
      <TouchableOpacity style={{alignItems:"center",width:100,borderRadius:20,backgroundColor:"white"}} onPress={() => {
        deleteEvent();
      ...
      <TouchableOpacity style={{alignItems:"center",borderRadius:20,width:100,backgroundColor:"white",marginTop:10}} onPress={() => {
