...
useEffect(()=>{ 
  ...
  setDgra(navigation.state.params.eventInfo.grad)
  setGrad(navigation.state.params.eventInfo.grad)
  setMjesto(navigation.state.params.eventInfo.mjesto)
  setVrijeme(navigation.state.params.eventInfo.vrijeme)
  
  console.log(navigation.state.params.eventInfo.datum +" je datum");
  
  const formattedDate = moment(navigation.state.params.eventInfo.datum, "MM/DD/YYYY, hh:mm:ss A");
  setDatum(formattedDate)

  setIzabro(true) 
  setTodayDate(mydate("date")) 
},[])
  ...
  return ( 
    ...
    height:"100%"}}>
    <TouchableOpacity onPress={()=>navigation.navigate("DetaljiOrg",{"id":navigation.state.params.eventInfo.eventid})}>
        ...
        <View>
            ...
            <TextInput style={styles.input} value={grad}  onChangeText={grad=>{
              ...
              {grad.length>0 && !izabro? <TouchableOpacity onPress={()=>{console.log(dgra);setIzabro(true);setGrad(dgra)}}>
    ...
    defaultButtonText={navigation.state.params.eventInfo.vrijeme.split(":")[0]}
...
<SelectDropdown
    ...
    defaultButtonText={navigation.state.params.eventInfo.vrijeme.split(":")[1]}
    dropdownStyle={styles.box2}
	data={minute}
	onSelect={(selectedItem, index) => {
...
/>
 ...
 selectedStartDate={datum} // Convert moment object to a JavaScript Date object
  disabledDates={(date) => {
    if (date < new Date(todayDate.split('-')[0], todayDate.split('-')[1] - 1, todayDate.split('-')[2])) {
      return true;
    }
    return false; // Make sure to return false for non-disabled dates
  }}
  selectedDayColor="#e7f2ff"
  onDateChange={(date) => {console.log(date);setDatum(date)}}
/>
            </View>
        </View>
        ...
        <TouchableOpacity onPress={()=>{
          ...
          if(izabro)navigation.navigate("EditInfo2",{eventInfo:navigation.state.params.eventInfo,grad,mjesto,datum,vrijeme,"brojLjudi":navigation.state.params.eventInfo.kolkoljudi,"sport":navigation.state.params.eventInfo.sport,"tip":navigation.state.params.eventInfo.tip}) 
          else 
          setErr("Izaberi grad!");}}>
