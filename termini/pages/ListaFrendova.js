...
export default function Start({navigation}) {
  ...
  function  obavijesti(name,surname) {
  


    setObavijest(name + " " + surname +" ti vise nije prijatelj")
    setTimeout(()=>{setObavijest("")},1000)
  
  }

  async function fetchaj(emailProp) {
      setEmail(emailProp +" JE MOJ EMAIL")
      console.log(emailProp)
    fetch(URL+"/userFrendovi/"+emailProp).then(res=>res.json()).then(data=>{setFrendovi(data);console.log(data);setSearchRes(data)})
  ...
  async function DeleteFrend(friendshipid,name,surname) { 
    obavijesti(name,surname)
    ...
    fetch(URL+"/deleteFrend",{method:"POST",body:JSON.stringify({email,friendshipid}),headers: {
...
{obavijest.length>0 ? <Text style={{color:"red"}}> {obavijest}</Text> : null}
    ...
    <ScrollView style={{maxHeight:"50%",backgroundColor:theme=="light"?"#e7f2ff":"#28559a",width:"80%",marginTop:20,borderColor:"gray",borderWidth:1,borderRadius:5}}>
        ...
        return (
          <View style={styles.frend} key={frend.friendshipid}>
            <Text style={{fontSize:20,color:theme=="dark"?"white":"black"}}>{frend.ime} {frend.prezime}</Text>
           
            <TouchableOpacity onPress={()=>DeleteFrend(frend.friendshipid,frend.ime,frend.prezime)}>
      ...
      }) : <View style={{justifyContent:"center",alignItems:"center",}}><Text style={{fontSize:17,marginTop:"50%"}}>Nemas frendova  :(</Text></View>}
